...
import type { StudentBillSubject } from "@/types/studentDashboard";
...
export interface StudentBillSubjectCardProps {
  subject: StudentBillSubject;
}

export function StudentBillSubjectCard({ subject }: StudentBillSubjectCardProps) {
  ...
  return (
    <Card>
      ...
      <AmountBlock>
        <Amount>{subject.amountFormatted}</Amount>
        <Sessions>
          {subject.sessionsCompleted} session{subject.sessionsCompleted === 1 ? "" : "s"} completed
        </Sessions>
      </AmountBlock>

      <Chips>
        <Chip $bg={COLORS.SURFACE_NEUTRAL_HOVER} $fg={DASHBOARD.TEXT_SECONDARY}>
          Held <ChipValue>{subject.sessionsCompleted}</ChipValue>
        </Chip>
